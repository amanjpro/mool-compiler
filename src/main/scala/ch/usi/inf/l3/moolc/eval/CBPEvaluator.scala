...
class CBPEvaluator(pgm: Program) {
	...
	private def specializeMethod(newName: String, cname: ClassName, mthd: String, 
							args: List[PEValue], store: ConstraintStore) : MMethod = {
	...
	private def getSpecializedClass(cname: ClassName, 
		...
		classBank.getOption(cname, args) match{
			...
			case None => 
				...
				val clazzStore = new ConstraintStore
	...
	private def specializeMethods(methods: List[MMethod], store: ConstraintStore) = {
	...
	private def pevalArgs(args: List[Expression], store: ConstraintStore) :
																					List[PEValue] = {
		val evalArgs = for(arg <- args) yield peval(arg, store)._2
		evalArgs
	}
	
	private def pevalArgs2(args: List[Expression], store: ConstraintStore) :
																					List[Expression] = {
	 ...
	 * 1- PE of primitive value constants always produces abstract values.
	 ...
	 * 7- When a variable is assigned to ⟂ that means it has not been yet initialized,
	 ...
	 */
	private def peval(expr: Expression, store: ConstraintStore) :
	 	 								 (Expression, PEValue) = {
		expr match {
			...
			case init @ New(c, argList, _) => 
				...
				if(args!= Nil && hasCT(args)){
					...
					val mobj = ObjectValue(MObject
              (specializedClazz.name, new ConstraintStore))
					(New(specializedClazz.name, newArgs, NoPosition), AbsValue(mobj))
				}
				else{
					val clazz = pgm.getClass(c).get
					val objStore = new ConstraintStore
			...
			case This(clazz, mthd, argList, _) => 
				...
				if(allCT(args)){
					val nenv = args zip method.args
					val newStore = new ConstraintStore
					newStore.addEnv(Map(nenv.map(_.swap): _*))
	 ...
	 * CT op _ = CT
	 */
	private def isCT(expr: Expression, store: ConstraintStore) = {
		true
	}
	 ...
	 * compilation
	 */
	private def checkStore(store: ConstraintStore, c: Expression, e2: Expression,
											e3: Expression) = {
		if(doCheckStore){
			doCheckStore = false
      ...
      */
      //store.makeConsistent(thenStore, elseStore)
			c match{
