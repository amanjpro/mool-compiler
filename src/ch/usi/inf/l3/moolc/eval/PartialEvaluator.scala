...
class PartialEvaluator(pgm: Program) {
	...
	private def specializeMethods(methods: List[MMethod], store: Store) = {
	...
	private def fevalArgs(args: List[Expression], store: Store) :
																				List[CTValue] = {
		val evalArgs = for(arg <- args) yield CTValue(feval(arg, store))
		evalArgs
	}
	
	private def pevalArgs(args: List[Expression], store: Store) :
																					List[PEValue] = {
		val evalArgs = for(arg <- args) yield peval(arg, store)._2
		evalArgs
	}
	
	private def pevalArgs2(args: List[Expression], store: Store) :
																					List[Expression] = {
	...
	private def feval(expr: Expression, store: Store) :
			Expression = {
		expr match{
			case x : Constant => x
	 ...
	 */
	private def peval(expr: Expression, store: Store) :
	 	 								 (Expression, PEValue) = {
		expr match {
	 ...
	 * CT op _ = CT
	 */
	private def isCT(expr: Expression, store: Store) = {
		true
	}
	
	/**
	 * According to the HPE paper I should throw an error if the branches changed
	 * incosistently, but I prefer to remove the differences and continue
	 * compilation
	 */
	private def checkStore(store: Store, c: Expression, e2: Expression,
											e3: Expression) = {
		if(doCheckStore){
			doCheckStore = false
